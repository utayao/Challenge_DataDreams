...
class Model:
    def __init__(self, fold, data_augment_params):
        ...
        self.td = TrainGenrator(FLAGS.data_path, subset=False)
    ...
    def fit(self, fold, is_training=True):

        # length = x_train.shape[0]
        total_loss = []
        total_acc = []
        summary = None
        for iter in range(len(FLAGS.iters)):
            ...
            batch_x, batch_y = self.td.next_batch(FLAGS.batch_size, is_training)
            _, loss, acc, summary = self.sess.run(
                [self.train_op, self.loss_op, self.correct_prediction, self.summaries_op], feed_dict={
                    self.x: batch_x,
                    self.y_: batch_y,
            ...
            if is_training and iter % 100 == 0 and iter:
